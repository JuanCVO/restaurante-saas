...
export const downloadSummaryPDF = async (req: Request, res: Response) => {
  try {
    ...
    doc.fontSize(12).font("Helvetica").fillColor("#666666").text("Reporte de ventas", { align: "center" });
    doc.moveDown(1.5);
    ...
    doc.font("Helvetica-Bold").fillColor("#27ae60")
      .text(`Total propinas:`, 50)
      .text(`$${totalPropinas.toLocaleString("es-CO")}`, 300, doc.y - doc.currentLineHeight());
    ...
    for (const summary of summaries) {
      ...
      doc.text(`  Efectivo: $${summary.efectivo.toLocaleString("es-CO")}   |   Datáfono: $${summary.datafono.toLocaleString("es-CO")}   |   Nequi: $${summary.nequi.toLocaleString("es-CO")}`);
      doc.fillColor("#27ae60")
        .text(`  Propinas del día: $${(summary.totalPropinas ?? 0).toLocaleString("es-CO")}`);
